...
double lennard_jones_poly<particle>::return_pair_potential( particle *part1 , particle *part2 ) {
  ...
  if( ( dist = part1->position.distance( image_part->position ) ) < cutoff_factor * actual_diameter ) {
    energy = ( pair_potential( part1 , image_part , dist , actual_diameter ) + SHIFT ) ;
  }
...
double lennard_jones_poly<particle>::compute_energy_contribution( void ) {
  ...
  for( int i=0 ; i<particles_in_list ; i++ ) {
    ...
    while( walker != NULL ) {
      verlet_list->god->nearest_image( walker->ptr , element[i].ptr , image_part ) ;      // I take the coordinates of the nearest copy of the neighbour. The list only one element for each couple of particles
      actual_diameter = element[i].ptr->radius + walker->ptr->radius ;
      if( ( dist = element[i].ptr->position.distance( image_part->position ) ) < cutoff_factor * actual_diameter ) {
	energy += ( pair_potential( element[i].ptr , image_part , dist , actual_diameter ) + SHIFT ) ;
      }
      walker = walker->next ;
    }
  }
...
inline double lennard_jones_poly<particle>::compute_energy_contribution( particle *part ) {
  ...
  if( part->list_ptr[idx] ) {
    ...
    while( walker != NULL ) {
      ...
      if( ( dist = part->position.distance( image_part.position ) ) < cutoff_factor * actual_diameter ) {
	energy += ( pair_potential( part , &image_part , dist , actual_diameter ) + SHIFT ) ;
      }
      walker = walker->next ;
    }
  }
...
void lennard_jones_poly<particle>::compute_forces_contribution(void) {
  ...
  double actual_diameter = 0 ;
  neighbour_list<particle> *walker = NULL ;
  particle *image_part = NULL , deltaF , **forces_t2 = verlet_list->god->forces_t2 ;
  image_part = new particle ;
  ...
  for( int i=0 ; i<particles_in_list ; i++ ) {
    ...
    while( walker != NULL ) {
      ...
      actual_diameter = element[i].ptr->radius + walker->ptr->radius ;
      if( ( dist = element[i].ptr->position.distance( image_part->position ) ) < cutoff_factor * actual_diameter ) {
	deltaF = pair_force( element[i].ptr , image_part , dist , actual_diameter ) ;
...
double lennard_jones_poly<particle>::compute_virial_contribution(void) {
  ...
  double actual_diameter = 0 ;
  neighbour_list<particle> *walker = NULL ;
  particle *image_part = NULL , deltaF ;
  image_part = new particle ;
  double virial = 0 ;
  ...
  for( int i=0 ; i<particles_in_list ; i++ ) {
	...
	virial += ( deltaF.position * ( element[i].ptr->position - image_part->position ) ) ;
      }
      walker = walker->next ;
    }
  }
...
void lennard_jones_poly<particle>::compute_per_part_energy_contribution( double *envec ) {
  ...
  for( int i=0 ; i<particles_in_list ; i++ ) {
    ...
    while( walker != NULL ) {
      ...
      actual_diameter = element[i].ptr->radius + walker->ptr->radius ;
      if( ( dist = element[i].ptr->position.distance( image_part->position ) ) < cutoff_factor * actual_diameter ) {
	energy = 0.5 * ( pair_potential( element[i].ptr , image_part , dist , actual_diameter ) + SHIFT ) ;
...
void lennard_jones_poly<particle>::compute_per_part_virial_contribution( double *virvec ) {
  ...
  for( int i=0 ; i<particles_in_list ; i++ ) {
	...
	virvec[ walker->ptr->id ] -= ( deltaF.position * image_part->position ) ;
      }
      walker = walker->next ;
    }
  }
