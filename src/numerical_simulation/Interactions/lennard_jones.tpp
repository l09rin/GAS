...
double lennard_jones<particle>::compute_energy_contribution( void ) {
  ...
  for( int i=0 ; i<particles_in_list ; i++ ) {
    ...
    while( walker != NULL ) {
      ...
      if( ( dist = element[i].ptr->position.distance( image_part->position ) ) < cutoff ) {
	energy += ( pair_potential( element[i].ptr , image_part , dist ) + SHIFT ) ;
      }
      walker = walker->next ;
    }
  }
...
void lennard_jones<particle>::compute_forces_contribution(void) {
  ...
  for( int i=0 ; i<particles_in_list ; i++ ) {
    ...
    while( walker != NULL ) {
      verlet_list->god->nearest_image( walker->ptr , element[i].ptr , image_part ) ;      // I take the coordinates of the nearest copy of the neighbour. The list only one element for each couple of particles
      if( ( dist = element[i].ptr->position.distance( image_part->position ) ) < cutoff ) {
	deltaF = pair_force( element[i].ptr , image_part , dist ) ;
...
void lennard_jones<particle>::compute_per_part_energy_contribution( double *envec ) {
  ...
  for( int i=0 ; i<particles_in_list ; i++ ) {
    ...
    while( walker != NULL ) {
      verlet_list->god->nearest_image( walker->ptr , element[i].ptr , image_part ) ;      // I take the coordinates of the nearest copy of the neighbour. The list only one element for each couple of particles
      if( ( dist = element[i].ptr->position.distance( image_part->position ) ) < cutoff ) {
	energy = 0.5 * ( pair_potential( element[i].ptr , image_part , dist ) + SHIFT ) ;
