...
double *configuration<particle>::min_part_distance( void ) {
  ...
  for( int k=0 ; k<INTERACTIONS.number ; k++ ) {
    ...
    for( int i=0 ; i<particles_in_list ; i++ ) {
      walker = element[i].neighbours ;
      while( walker != NULL ) {
	nearest_image( walker->ptr , element[i].ptr , image_part ) ;      // I take the coordinates of the nearest copy of the neighbour. The list only one element for each couple of particles
	dist = element[i].ptr->distance( image_part ) ;
	if ( dist < min ) {
	  min = dist ;
