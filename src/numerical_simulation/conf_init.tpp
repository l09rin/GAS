...
void configuration<particle>::generate_ControlledEnergy_randomConfig( void ) {
  cout << " Generating random positions of particles via controlled energy insertion" << endl ;
  cout << "   sample interaction potential: repulsive WCA" << endl ;
  ...
  for( int i=0 ; i<numberOfPart ; i++ ) {
    ...
    do {
      ...
      too_high_energy = 0 ;
      for( int j=0; j<i; j++) {
	nearest_image( particles[j] , particles[i] , image_part ) ;                     // PBC
	distance = particles[i]->position.distance( image_part->position ) ;
	...
	for( int cy=-1; cy<2 && (!keep_trying); cy++ ) {
	  for( int cz=min_cz; cz<max_cz && (!keep_trying); cz++ ) {
	    ...
	    while( walker != NULL ) {          // this cycle scans the list of a cell to individuate neighbours for i particle
	      nearest_image( particles[walker->host] , particles[i] , image_part ) ;
	      if( particles[i]->position.distance(image_part->position) < 2.0*radius ) keep_trying = 1 ;
	      walker = walker->next ;
	    }
	  }
	}
...
void configuration<particle>::generate_config_by_file( const char *file_name , const char *format ) {
  ...
  if( data_file.fail() ) {
    ...
    cout << "Configuration loading failed ! You need to specify the file format (xyz) !" << endl ;
    exit( EXIT_FAILURE ) ;
  }
